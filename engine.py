...
class CustomSegmentationWithYOLO:
    ...
    def generate_mask_from_result(self, results):
        for result in results:
            if result.masks:
                masks = result.masks.data # Get the mask tensor
                boxes = result.boxes.data # Get the bounding boxes

                # Extract class labels from the bounding boxes
                clss = boxes[:, 5]
                
                # 0 class stands for people in COCO
                ...
                # scale to visualize results
                # white (255) represents detected objects
